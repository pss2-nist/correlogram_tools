...
def sim_moire(experiment, save_raw=False, ext='tif', verbose=True):
    ...
    * no wavelength dispersion or angular divergence
    ...
    print(f"Blur_method{blur_method}")
    periods = set()
    file_index = 1
...
def geoblur_pinhole(
    ...
    print(f"pinhole parameters: r:{r}, lg:{lambda_g}, D:{D}, Z:{Z}, width:{width},kernel:{np.unique(kernel)}")
    print(image.shape, blurred.shape, len(np.unique(image-blurred)))
    return blurred
